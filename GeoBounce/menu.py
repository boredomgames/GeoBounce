# See gui.py and its usage in __main__.py, ask @HereIsKevin on project board
# for details about gui.py design and system plus anything else...
...
class MenuItem(object):
    ...
    def on_click(self, click_pos):
        if self._coords[0] < click_pos[0] < (
            self._coords[0] + self._dimensions[0]
        ) and self._coords[1] < click_pos[1] < (
            self._coords[1] + self._dimensions[1]
        ):
            self._action()
            return True
        else:
