...
class LevelOptimized(object):
    ...
    def draw_current(self):
        ...
        obstacles = filter(
            lambda item: item.bbox()[0]
            <= self._current_coords + window_width + 100,
            self._obstacles,
        )
        ...
        if self._player_jumping and self._player_jump_frames < JUMP_FRAMES:
            ...
            if self._player_jump_frames < JUMP_FRAMES / 2:
                ...
                self._player_drawn[0].move(
                    (0, -JUMP_PATH[self._player_jump_frames])
                )
            else:
    ...
    def collide(self):
        ...
        for item in self._player_drawn[0].collide():
            ...
            if item in self._surfaces_tags:
                ...
                surface = self._surfaces_drawn[
                    self._surfaces_tags.index(item)
                ].bbox()
